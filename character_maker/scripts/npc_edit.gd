...
#	var c7: Color = NpcMaker.SHOES_COLOR[randi()%NpcMaker.SHOES_COLOR.size()]
	...
	# clothes
	if randf() > .6:
		var t10: String = NpcMaker.BODY_PARTS[npc_type].bodysuit[
			randi() % NpcMaker.BODY_PARTS[npc_type].bodysuit.size()
		]
...
func set_color(item_type: String, new_color: Color):
	if npc_description.has(item_type):
		...
		npc_id.set_part(
			npc_description[item_type].key,
			npc_description[item_type].albedo,
			npc_description[item_type].roughness,
			npc_description[item_type].metalness
		)
	...
	if npc_description.has(item_type):
		...
		if NpcMaker.BODY_PARTS[npc_type].has(item_type):
			if NpcMaker.BODY_PARTS[npc_type][item_type].size() > 0:
				npc_description[item_type] = {
					"key": NpcMaker.BODY_PARTS[npc_type][item_type][0],
					"albedo": "000000",
					"roughness": 1.0,
					"metalness": 0.0
				}
...
#		print(NpcMaker.BODY_PARTS[npc_type][item_type])
		npc_description[NpcMaker.BODY_PARTS[npc_type][item_type][item_number]] = {
			"key": NpcMaker.BODY_PARTS[npc_type][item_type][item_number], "albedo": cc, "roughness": rr, "metalness": mm
		}
